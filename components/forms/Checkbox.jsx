...
export function Checkbox({ label, checked, onChange, pigment = 'var(--pigment-red)', disabled, style, ...rest }) {
  return (
    <label style={{ display: 'inline-flex', alignItems: 'flex-start', gap: 'var(--space-3)', cursor: disabled ? 'default' : 'pointer', opacity: disabled ? 0.4 : 1, ...style }}>
      <input {...rest} type="checkbox" checked={checked} onChange={onChange} disabled={disabled} style={{ position: 'absolute', opacity: 0, width: 0, height: 0 }} />
